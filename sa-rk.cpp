...
int main(int argc, char** argv){
    ...
    if(input_pos.compare("")==0){
    	...
    	FILE * fp = fopen(input_pos.c_str(), "rb");

		if (fp == NULL){

		  cout << "Error while opening positions file " << input_pos <<endl;
		  exit(0);
		...
		//read how many uint64_t are stored in the file
		auto bytes = fread(&s, sizeof(uint64_t), 1, fp);
		assert(bytes);

		SA = vector<uint64_t>(s);

		//read positions to sort
		bytes = fread(SA.data(), sizeof(uint64_t), s, fp);
		assert(bytes);
